...
|  | iOS | Android |
| :--: | :--: | :--: |
| 現在 | ![ios/statusbar](ios/statusbar.png) <br/> [Status bar](https://developer.apple.com/jp/design/human-interface-guidelines/status-bars) | ![android/statusbar](android/statusbar.png)  <br/> [Status bar](https://developer.android.com/design/ui/mobile/guides/foundations/system-bars#status-bar) |
...
基本的に開発側でカスタムできることは少ないです。動画プレーヤーやゲームアプリなど全画面で表示したいアプリの場合は、隠したりすることは可能です。
...
Androidの`Gesture bar`という名前も正式なガイドラインにある呼称ではなく、いまだにGoogleの一部記事では`Navigation bar`と書かれています。しかしMaterial Design 3において`Navigation bar`は後述するタブUIをことを指すことになったので、今後これは明確に間違いになります。
...
この次の「グローバルナビゲーションのタブバー」と同じ位置に表示されて似ていますが、明確に別のUIです。
...
Appleではこのような<b>自動で消える</b>トースト系の通知UIはNativeに存在しません。カスタムで似たようなUIを実装しているアプリはよくあり、慣例的に`HUD`(Head Up Display)や`Toast`と呼ばれることが多いようです。ちなみにNativeの通知UIだと[Alerts](https://developer.apple.com/design/human-interface-guidelines/alerts), [Action sheets](https://developer.apple.com/design/human-interface-guidelines/action-sheets)などがあります。
...
`Tab bar`同様にアプリ内でのセクションを移動するためのナビゲーションUIです。iPadOSでは`Tab bars`は上部に表示するか、代わりにこの`Sidebar`を使うことを推奨されます。`Tab bar`と`Sidebar`を統合する実装も可能で、ユーザーが切り替えて利用できるようになります。

https://developer.apple.com/design/human-interface-guidelines/tab-bars#iPadOS

`Sidebar`はiPadOS, macOS, visionOSでの利用が想定されており、**iOS(iPhone)では非推奨なので注意**です。iOSで同様の目的がある場合はTabbarを使うか、[Sheet](https://developer.apple.com/design/human-interface-guidelines/sheets)を使うのが適切です。
...
* Apple系OSにおいてはアプリ上部のBarUI (※ただし慣例名、正式名は`Toolbars`)
* **現在の**AndroidOSにおいては下部のタブUI 
* **以前の**AndroidOSにおいてはシステムのホームに戻るUI
